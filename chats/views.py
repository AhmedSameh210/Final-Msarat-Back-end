...
def send_message(request):
                ...
                if options == 'أ' :
                    ...
                    return Response({
                        'message': (
                            ...
                            f" اضغط على 'ابدأ ' وابدأ رحلتك نحو التميز والإبداع."
                        ),
                        'content': content,
                        'lesson_id': lesson_id
                    })
    ...
    if not progress_state:
        ...
        # Save the chat instance to persist changes in the database
        chat.save()
        return Response({'message': 'مرحبا بك في درس فيثاغورث . للمتابعه اضغط زر التالي', 'lesson_id':lesson_id , 'content': ''})
    ...
    if (ChatProgress.EXPLANATION_EXAM.value in progress_state and
        ...
        if explanation_exam_result["passed"] == True:
            ...
            if not previous_content:  # Checks if the list is empty
                return Response({'message': '''عمل رائع!

                لقد أكملت الأسئلة بنجاح.

                اضغط على "التالي " للمتابعة!''', 'content': [], 'lesson_id': lesson_id})
            else:  # If the list is not empty
                return Response({'message': '''عمل رائع!

                لقد أكملت الأسئلة بنجاح.

                لاحظنا وجود بعض المفاهيم السابقة التي تحتاج إلى مراجعة.

                سنعرض لك الآن محتوى مخصصًا لهذه المفاهيم.

                اضغط على "ابدأ" للمتابعة!''', 'content': previous_content, 'lesson_id': lesson_id})
    ...
    if ChatProgress.PREVIOUS_CONTENT_CONTENT.value in progress_state and progress_state[ChatProgress.PREVIOUS_CONTENT_CONTENT.value]["status"] == "pending":
        ...
        if not prev_revision_exam:  # Check if the list is empty or None
            return Response({'message': '''ونظرًا لإتقانك المفاهيم السابقة المرتبطة بالدرس

سوف ننتقل مباشرةً إلى المرحلة التالية.

اضغط على "التالي" للمتابعة!''', 'content': prev_revision_exam, 'lesson_id': lesson_id})
    ...
    if ChatProgress.PREV_REVISION_EXAM.value in progress_state and progress_state[ChatProgress.PREV_REVISION_EXAM.value]["status"] in ["pending", "failed"] :
        ...
        if prev_revision_exam_result["passed"] == True or progress_state[ChatProgress.PREV_REVISION_EXAM.value].get("attempts", 0) >= 2:
            ...
            prev_revision_exam = intro_exam.get_failed_topics_for_previous_exam()
            if not prev_revision_exam:  # Check if the list is empty or None
                return Response({'message': '''الآن، سنعرض عليك مثالاً عمليًا يُظهر استخدام المفاهيم التي تعلمتها في حل مشكلة من واقع الحياة.''', 
                                'content': practice_videos_content, 
                                'lesson_id': lesson_id})

            return Response({'message': '''رائع! لقد أنهيت تقييم المفاهيم الأساسية بنجاح. 
            الآن، سنعرض عليك مثالاً عمليًا يُظهر استخدام المفاهيم التي تعلمتها في حل مشكلة من واقع الحياة.''', 
                            'content': practice_videos_content, 
                            'lesson_id': lesson_id})        
    ...
    if ChatProgress.FINAL_ASSESSMENT_EXAM.value in progress_state and progress_state[ChatProgress.FINAL_ASSESSMENT_EXAM.value]["status"] == "pending":
            ...
            if exam_result["passed"] == True:
                ...
                if exam_result['percentage'] < 50:
                    ...
                    # Check if the failed_topics_content is empty or None
                    if not failed_topics_content:
                        # Remove the "ج" option if failed_topics_content is empty or None
                        options = [
                            {"أ": "خروج"},
                            {"ب": "الدرس التالي"}
                        ]
                    ...
                    # Check if the "ج" option exists in the options list before appending it to content
                    if len(options) > 2:
                        content += f"جـ) {options[2]['ج']}\n"
                    ...
                    # Check if the "ج" option exists in the options list before adding it to the content
                    if len(options) > 2:
                        content += f"جـ) {options[2]['ج']}\n"
                    ...
                    if not failed_topics_content:
                        ...
                        # Include the "ج" option if failed_topics_content is not empty
                        options = [
                            {"أ": "خروج"},
                            {"ب": "الدرس التالي"},
                            {"ج": "تحسين المستوي"}
                    ...
                    # Add the "د" option at the end in both cases
                    options.append({"د": "اسأله التحدي"})
                    ...
                    # Dynamically append options to the content
